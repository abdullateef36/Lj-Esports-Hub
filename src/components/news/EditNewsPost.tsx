...
import { Newspaper, X, Upload } from "lucide-react";
import { NewsPost, NewsCategory } from "@/lib/news";
...
export default function EditNewsPost({ post, onClose }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await updateDoc(doc(db, "news-posts", post.id), {
        title,
        content,
        excerpt,
        coverImage,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 px-4 overflow-y-auto py-8">
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="text-3xl font-heading font-bold text-white uppercase">Edit News Post</h2>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              {coverImage && (
                <div className="flex-1 relative h-62.5 sm:h-full">
                    <Image
                    src={coverImage}
                    alt="Cover preview"
                    fill
          ...
          >
            {isSubmitting ? "Updating..." : "Update News Post"}
          </button>
        </div>
      </div>
    </div>
